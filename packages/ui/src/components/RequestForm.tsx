...
import { InternalRoute, fillPathParams, AuthConfig } from '../utils/api';
...
const RequestForm: React.FC<RequestFormProps> = ({
  ...
  const buildUrl = () => {
    ...
    if (authConfig && authConfig.type === 'apikey' && authConfig.apiKeyName && authConfig.apiKeyValue && authConfig.apiKeyIn === 'query') {
      url.searchParams.set(authConfig.apiKeyName, authConfig.apiKeyValue);
    }
    return url.toString();
  };
  ...
  const handleCopyCurl = () => {
    ...
    if (authConfig) {
      if (authConfig.type === 'bearer' && authConfig.token && !hasManualAuth) {
        headersList.push(`-H "Authorization: Bearer ${authConfig.token}"`);
      } else if (authConfig.type === 'apikey' && authConfig.apiKeyName && authConfig.apiKeyValue) {
        if (authConfig.apiKeyIn === 'header') {
          headersList.push(`-H "${authConfig.apiKeyName}: ${authConfig.apiKeyValue}"`);
        }
        // If in query, it's appended below in the URL directly (actually it should be in buildUrl, I'll update buildUrl shortly)
      } else if (authConfig.type === 'basic' && authConfig.basicUser) {
        const encoded = btoa(`${authConfig.basicUser}:${authConfig.basicPass ?? ''}`);
        headersList.push(`-H "Authorization: Basic ${encoded}"`);
      }
    }
    ...
    if (['POST', 'PUT', 'PATCH'].includes(route.method)) {
      ...
      } else if (contentType === 'application/x-www-form-urlencoded' && formFields.some(f => f.key)) {
        if (!headerParams.some((h) => h.key.toLowerCase() === 'content-type')) {
          headersList.push(`-H "Content-Type: application/x-www-form-urlencoded"`);
        }
        const params = new URLSearchParams();
        formFields.forEach(({ key, value }) => {
          if (key) params.append(key, value);
        });
        bodyStr = ` -d '${params.toString().replace(/'/g, "'\\''")}'`;
      }
    }
    ...
    try {
      ...
      if (authConfig) {
        if (authConfig.type === 'bearer' && authConfig.token && !hasManualAuth) {
          ...
          if (authConfig.apiKeyIn === 'header') {
            headers.set(authConfig.apiKeyName, authConfig.apiKeyValue);
          }
        } else if (authConfig.type === 'basic' && authConfig.basicUser) {
          const encoded = btoa(`${authConfig.basicUser}:${authConfig.basicPass ?? ''}`);
  ...
  return (
    <div className="space-y-4 pt-1">
      {/* Path params */}
      {paramNames.length > 0 && (
        <div className={`bg-gray-50/80 dark:bg-zinc-800/60 border border-gray-300 dark:border-zinc-700/60 rounded-xl p-4 space-y-3 ${lockedClass}`}>
          <strong className="block text-xs font-bold uppercase tracking-wider text-gray-800 dark:text-gray-200">
            Path Parameters
          </strong>
          {paramNames.map((name) => (
            <div key={name} className="flex items-center space-x-3">
              <span className="w-28 text-xs font-mono text-gray-900 dark:text-gray-200 shrink-0 font-semibold">
                :{name}
              </span>
              <input
                type="text"
                ...
                className="flex-1 px-3 py-1.5 text-sm border-b-2 border-gray-300 dark:border-zinc-700 bg-transparent text-gray-950 dark:text-gray-100 focus:outline-none focus:border-fastapi focus:ring-0 font-mono transition-colors placeholder:text-gray-500 dark:placeholder:text-gray-500"
              />
            </div>
          ))}
        </div>
      ...
      <div className={`bg-gray-50/80 dark:bg-zinc-800/60 border border-gray-300 dark:border-zinc-700/60 rounded-xl p-4 space-y-3 ${lockedClass}`}>
          ...
          <div key={i} className="flex items-center space-x-3">
            ...
            {isTryItOut && (
              <button
                ...
                className="text-gray-500 hover:text-rose-600 dark:text-gray-400 dark:hover:text-rose-400 p-1.5 transition-colors shrink-0"
        ...
        {isTryItOut && (
          <button
            ...
            className="text-xs font-bold text-fastapi hover:text-fastapi-hover transition"
          >
            + Add query parameter
          </button>
        )}
      ...
      <div className={`bg-gray-50/80 dark:bg-zinc-800/60 border border-gray-300 dark:border-zinc-700/60 rounded-xl p-4 space-y-3 ${lockedClass}`}>
        ...
        {authConfig && authConfig.type !== 'none' && isTryItOut && (
          <p className="text-xs italic text-gray-600 dark:text-gray-400 pt-1 font-medium">
            Global authorization ({authConfig.type}) will be applied
          </p>
        )}
      </div>
      ...
      {['POST', 'PUT', 'PATCH'].includes(route.method) && (
        <div className={`bg-gray-50/80 dark:bg-zinc-800/60 border border-gray-300 dark:border-zinc-700/60 rounded-xl p-4 space-y-3 ${lockedClass}`}>
          <div className="flex items-center space-x-2 mb-3">
            <span className="text-[11px] font-bold uppercase tracking-wider text-gray-800 dark:text-gray-200">Content-Type:</span>
            {(['application/json', 'application/x-www-form-urlencoded'] as ContentType[]).map((ct) => (
              <button
                key={ct}
                type="button"
                ...
                className={`text-[11px] px-2.5 py-1 rounded-lg font-mono font-medium transition-all ${
                  contentType === ct
                    ? 'bg-fastapi text-white'
                    : 'bg-gray-200 dark:bg-zinc-800 text-gray-800 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-zinc-700'
                }`}
              >
                {ct === 'application/json' ? 'JSON' : 'Form'}
              </button>
            ))}
          </div>
          ...
          {contentType === 'application/json' ? (
            <textarea
              ...
              className="w-full p-3 text-sm font-mono border-b-2 border-gray-300 dark:border-zinc-700 bg-transparent text-gray-950 dark:text-gray-100 focus:outline-none focus:border-fastapi focus:ring-0 transition-colors placeholder:text-gray-500 dark:placeholder:text-gray-500"
              placeholder='{\n  "key": "value"\n}'
            />
          ) : (
              ...
              {formFields.map((kv, i) => (
                <div key={i} className="flex items-center space-x-3">
                  <input
                    ...
                    value={kv.key}
                    onChange={(e) =>
                      handleKeyValueChange(formFields, setFormFields, i, 'key', e.target.value)
                    }
                    className="flex-1 px-3 py-1.5 text-sm border-b-2 border-gray-300 dark:border-zinc-700 bg-transparent text-gray-950 dark:text-gray-100 focus:outline-none focus:border-fastapi focus:ring-0 font-mono transition-colors placeholder:text-gray-500 dark:placeholder:text-gray-500"
                  />
                  <input
                    ...
                    value={kv.value}
                    onChange={(e) =>
                      handleKeyValueChange(formFields, setFormFields, i, 'value', e.target.value)
                    }
                    className="flex-1 px-3 py-1.5 text-sm border-b-2 border-gray-300 dark:border-zinc-700 bg-transparent text-gray-950 dark:text-gray-100 focus:outline-none focus:border-fastapi focus:ring-0 font-mono transition-colors placeholder:text-gray-500 dark:placeholder:text-gray-500"
                  />
                  {isTryItOut && (
                    <button
                      type="button"
                      onClick={() => removeRow(setFormFields, i)}
                      className="text-gray-500 hover:text-rose-600 dark:text-gray-400 dark:hover:text-rose-400 p-1.5 transition-colors shrink-0"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
      ...
      {isTryItOut && (
        <div className="bg-gray-50/80 dark:bg-zinc-800/60 border border-gray-300 dark:border-zinc-700/60 rounded-xl px-4 py-3 flex items-center space-x-3">
          <Badge method={route.method} />
          <code className="text-xs font-mono text-gray-900 dark:text-gray-200 break-all flex-1 font-semibold">
            {buildUrl()}
          </code>
        </div>
      )}
      ...
      {isTryItOut && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 pt-1">
          <button
            ...
            onClick={sendRequest}
            disabled={sending}
            className={`w-full sm:w-auto px-7 py-2.5 bg-swagger-get hover:bg-sky-600 text-white text-sm font-bold rounded-lg shadow-sm disabled:opacity-75 transition-all flex items-center justify-center space-x-2 ${
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2.5 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-900 dark:text-gray-100 text-sm font-semibold rounded-xl border border-gray-300 dark:border-zinc-700 transition-all flex items-center justify-center space-x-2"
          >
            <span>{copiedCurl ? 'cURL Copied! ✓' : 'Copy cURL'}</span>
          </button>

          {error && <span className="text-rose-600 dark:text-rose-400 text-sm font-bold">{error}</span>}
        </div>
      )}

      {/* Response Display */}
      {response && (
        <div className="animate-slideUp mt-6 border border-gray-300 dark:border-zinc-800 rounded-2xl bg-zinc-950 text-gray-100 overflow-hidden shadow-2xl">
          {/* Status Header */}
          ...
          {/* Response Headers */}
          <div className="p-4 border-b border-zinc-800/60 bg-zinc-950/80">
      ...
      {isTryItOut && history.length > 0 && (
        <div className="mt-6 border border-gray-300 dark:border-zinc-800 rounded-2xl bg-white dark:bg-zinc-900 p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold text-gray-600 hover:text-rose-600 dark:text-gray-400 dark:hover:text-rose-400 transition"
            >
              Clear history
            </button>
          </div>
          ...
          <div className="space-y-2">
            {history.map((entry) => {
              ...
              return (
                <div
                  key={entry.id}
                  className="border border-gray-300 dark:border-zinc-700/60 rounded-xl overflow-hidden bg-white dark:bg-zinc-800/60 shadow-2xs"
                >
                  <div
                    onClick={() => setExpandedHistoryId(isExpanded ? null : entry.id)}
                    className="flex items-center justify-between p-3 cursor-pointer hover:bg-gray-100/70 dark:hover:bg-zinc-800 transition-colors text-xs font-mono"
                  >
                    <div className="flex items-center space-x-3">
                      <span
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <svg
                        className={`w-3.5 h-3.5 text-gray-600 dark:text-gray-400 transition-transform duration-200 ${
                  ...
                  {isExpanded && (
                    <div className="border-t border-gray-300 dark:border-zinc-800 bg-zinc-950 text-gray-100 overflow-hidden">
                      <div className="p-3 border-b border-zinc-800/60 bg-zinc-950/80">
